...
| `EnqueueSnapshot` / `EnqueueMultiSnapshot` | Redis 已执行入队脚本，并通过 `WAITAOF 1 0 2000` 确认本机 AOF；尚不代表 PostgreSQL 已提交。部署配置 `backlog.enqueueAck: "memory"` 时不等待 `WAITAOF`，只代表已写入 Redis 内存，Redis 崩溃可能丢失约1秒已确认入队 |
