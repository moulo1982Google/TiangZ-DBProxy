...
2. **客户端连接存在队头阻塞。** 单条 TCP 连接按请求串行，RecordKey 稳定映射到固定连接。后端故障导致慢请求占住连接后，会拖慢同连接上的其他玩家请求。短期可按读写用途分池并调整池大小；长期若确有容量证据，再评估协议多路复用，避免先引入复杂度。（2026-09-19：写法长稳显示客户端单连接单请求成为瓶颈，已实现同连接多请求在途并保留同一记录顺序，见[网络协议](network-protocol.md#连接并发和-endpoint)。）
