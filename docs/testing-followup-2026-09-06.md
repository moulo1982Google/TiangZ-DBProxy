...
## 09-07 提交后分段延迟实测（r12，已完成）

DBProxy `69ef4f9`、TiangZ `ba2bc3d` 提交后从干净工作区串行重建 release，正常业务包/codegen 重建通过，生成物与协议/Core API 锁没有变化。DBProxy 默认工作区 124 项、显式真实 PG/Redis 测试 27 项、控制器与原生资源采集 7 项全部通过。

`20260907-latency-30m-r12` 于北京时间 2026-09-07 09:43:55 开始 1,800 秒负载，10:14:15 最终对账通过，控制器退出 0，保存证据审计再次通过。100 游戏玩家 + 独立 100 持久化探针 + 通用 Relay；五次故障为 cache / PostgreSQL / 可靠 Redis / AOF / cache，各有原账号两轮恢复及交易复核。本轮不含 MapHost/Location/DBProxy 进程和动态副本故障，不替代此前完整矩阵。

- 199,362 次读取，旧读/缺失/不变量错误均 0；排队写成功 33,708 次、错误 492 次，事务首次应用 16,165 次、重复 2 次、错误 417 次。旧 Trade 首次应用 1,296 次、重复 2 次、错误 2 次。19 次读取领先本地版本保留，最终收敛通过。
- 795 条通用事件全部发布，实际 Stream 去重与顺序校验通过，死信 0；1,590 份快照和 795 条不可变事实匹配，2 笔游戏交易审计、事件和资产复核通过。Relay 保留 12 次重试。
- AOF 专项的 64 条已确认记录重启后逐条 PG 核对通过。故障窗口有 7 份不健康游戏分片结果；PG 恢复初期一次登录 10003，原账号重试后两轮通过，不能写成全程无错误。
- 最低可用内存 3.454 GiB，CPU 峰值 67.01%，资源守卫触发 0；日志丢弃与指标缺失均 0，证据约 149.8 MiB（含冻结二进制），不是 7 天日志容量证明。

新分段指标有效区分了两种问题。两次缓存停机均复现约 4 秒游戏尾延迟：cache_write / committed_cache_sync 的 p99 在 2–5 秒桶，客户端连接排队同时升高；同窗口两节点 PG operation 与缓存修复 ACK 均没有超过 1 秒样本。PG 停机窗口则以共享连接等待突出：09:52:12–09:53:12 主节点 PG 排队均值约 4.62 秒、p99 在 10–15 秒桶，持锁操作 p99 在 2–5 秒桶。这不是纯 SQL 耗时，也不能把阶段分位数相加或相减。

下一项优先评估缓存不可用时的快速降级/有界同步预算，其次是 PG 故障时的连接排队控制；必须保留现有持久化修复、revision、幂等及结果未知恢复契约。本轮只提交和验证指标，不修改性能策略。正常窗口未复现旧夜间秒级尾延迟，但资源压力不同，不能认定旧问题全部修复。

完整采样口径、时间窗和错误证据见相邻 TiangZ `docs/testing/latency-attribution.md` 第三步；原始文件在工作区 `.build-tmp/local-validation/20260907-latency-30m-r12/`，不提交原始日志或凭据。仅重置两个专用测试库及两只演练 Redis，没有另外备份被重置测试数据；原 tiangz 数据库、外网、历史证据与 WoW335 不动。受管进程已退出，测试数据库容器恢复健康并保留数据。本步骤没有重跑完整发布门禁/7 天长稳或 WoW335 实机验收。
